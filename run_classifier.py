...
flags.DEFINE_string(
    "model_dir", None,
    "The output directory where the model checkpoints already have been written.")
...
flags.DEFINE_string(
    "do_arbitrary", '',
    "Generate results for an arbitrary file in `data_dir`")

flags.DEFINE_string(
    "model_name", '',
    "The name of the model `data_dir`")
...
### partial input and other settings

flags.DEFINE_bool(
    "partial_input", False,
    "Remove hypothesis from input (in the case of NLI)")

flags.DEFINE_float(
    "max_input", float('inf'),
    "The maximum number of training examples to allow")
...
# 调整 convert_single_example 函数，确保它在处理预测数据时不会尝试访问标签
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer):
    ...
    return InputFeatures(
        ...
        segment_ids=segment_ids,
        label_id=label_id,
        is_real_example=example.label is not None)
...
def main(_):
  ...
  processors = {
      "nli4ct": NLI4CTProcessor,
  }

  # train_examples = processors["cmnli"]().get_dev_examples(FLAGS.data_dir,
  #                                                             partial_input=FLAGS.partial_input,
  #                                                             )
  # exit('exited..')
  ...
  if not FLAGS.do_train and not FLAGS.do_eval and not FLAGS.do_predict and not FLAGS.do_arbitrary:
    raise ValueError(
  ...
  run_config = tf.contrib.tpu.RunConfig(
      ...
      keep_checkpoint_max=FLAGS.keep_checkpoint_max, ## added by Hai
  ...
  ## add by Kyle. 
  if FLAGS.do_arbitrary:
    tf.logging.info("Running arbitrary eval for: %s" % FLAGS.do_arbitrary)
    eval_examples = processor.get_arbitrary_examples(FLAGS.data_dir,
                                                       FLAGS.do_arbitrary,
                                                       partial_input=FLAGS.partial_input)
    tf.logging.info('Built the target data')
    ...
    steps_and_files = [[1,FLAGS.model_dir]]
    metrics_out = os.path.join(FLAGS.output_dir, "metrics.json")
    tf.logging.info('metrics file: %s' % metrics_out)
    for global_step, filename in sorted(steps_and_files, key=lambda x: x[0]):
      tf.logging.info('Evaluating: %s' % filename)
      result = estimator.evaluate(input_fn=eval_input_fn,
                                    steps=eval_steps, checkpoint_path=filename)
      tf.logging.info("***** Eval results %s *****" % (filename))
      for key in sorted(result.keys()):
      ...
      with open(metrics_out,'w') as metrics:
        outgoing = {
          "evaluation_accuracy" : float(result["eval_accuracy"]),
          "evaluation_loss"     : float(result["eval_loss"]),
        }
        metrics.write(json.dumps(outgoing,indent=4))
        metrics.write("\n")
        
  if FLAGS.do_predict:
    predict_examples = processor.get_test_examples(FLAGS.data_dir, partial_input=FLAGS.partial_input)

    num_actual_predict_examples = len(predict_examples)
    if FLAGS.use_tpu:
        # 对于TPU，需要确保输入的batch size是固定的，因此需要填充predict_examples到batch size的倍数。
    ...
    index2label_map = {i: label for i, label in enumerate(label_list)}
    output_predict_file_label_name = "nli4ct_predict.json"
    ...
    with tf.gfile.GFile(output_predict_file, "w") as writer:
        with tf.gfile.GFile(output_predict_file_label, "w") as writer_label:
            ...
            for i, prediction in enumerate(result):
                ...
                test_label_dict = {
                    "id": i,
                    "label": str(index2label_map[label_index])
                }
